...
class HazardPerception{
    ...
    public $videoLocation = 'https://www.car-theory.co.uk/videos/';
     ...
     */
    public function __construct(Database $db, $template, $userID){
        self::$db = $db;
        $this->userID = $userID;
        self::$template = $template;
        self::$template->caching = false;
        self::$template->force_compile = true;
        self::$template->addTemplateDir(LAYOUT_PATH.'student'.DS.'templates'.DS.'hazard');
    }
    ...
    public function getUserProgress($testID){
        if($_SESSION['hptest'.$this->getTestID()]){
            ...
            $userProgress = self::$db->select($this->progressTable, array('user_id' => $this->getUserID(), 'test_id' => $testID));
    ...
    protected function prevVideo($videoID){
        ...
        if($_GET['review']){
            if(is_numeric($_GET['pupil'])){$pupil = 'pupil=3688&amp;';}
            if($prevID >= 1){$vidURL = '/student/hazard?'.$pupil.'review='.$_GET['review'].'&amp;prim='.$_SESSION['hptest'.$this->getTestID()]['videos'][$prevID];}
            else{$vidURL = '/student/hazard?'.$pupil.'report=true&amp;id='.$_GET['review'];}
            return '<a href="'.$vidURL.'" class="prevvideo"><span class="sr-only">Prev Clip</span></a>';
        }
        else{
    ...
    protected function nextVideo($videoID){
        ...
        if($_GET['review']){
            if(is_numeric($_GET['pupil'])){$pupil = 'pupil=3688&amp;';}
            if($nextID <= 14){$vidID = $vidURL = '/student/hazard?'.$pupil.'review='.$_GET['review'].'&amp;prim='.$_SESSION['hptest'.$this->getTestID()]['videos'][$nextID];}
            else{$vidURL = '/student/hazard?'.$pupil.'report=true&amp;id='.$_GET['review'];}
            return '<a href="'.$vidURL.'" class="nextvideo"><span class="sr-only">Skip Clip</span></a>';
        }
        else{
     ...
     */
    protected function getScript($videoID = false){
        if($this->report == false){return '<script type="text/javascript" src="/studentscript/scripts/hazard-perception-hazupdate.js?vid='.$videoID.'"></script>';}
        else{return '<script type="text/javascript" src="/studentscript/scripts/hazard-report-hazupdate.js"></script>';}
    }
    ...
    public function createHTML($prim, $review = false){
        ...
        if(is_numeric($prim)){
            ...
            self::$template->assign('videotitle', $videoInfo['title']."<br />".$videoInfo['title2'], true);
            self::$template->assign('videodesc', nl2br($videoInfo['description']."\r\n\r\n".$videoInfo['description2']), true);
            self::$template->assign('no_questions', $this->numVideos, true);
            self::$template->assign('your_score', $this->clipScore($prim), true);
            self::$template->assign('anti_cheat', $this->anyCheating($prim), true);
            self::$template->assign('question_no', $this->currentVideoNo($prim), true);
            self::$template->assign('vid_id', $prim, true);
            self::$template->assign('video', $this->getVideo($prim), true);
            self::$template->assign('prev_question', $this->prevVideo($prim), true);
            self::$template->assign('next_question', $this->nextVideo($prim), true);
            self::$template->assign('score_window', $this->getScoreWindows(), true);
            self::$template->assign('review_flags', $this->getReviewFlags($prim), true);
            self::$template->assign('script', $this->getScript($prim), true);
            self::$template->assign('testID', $this->getTestID(), true);
    ...
    protected function anyCompleteTests(){
        return self::$db->select($this->progressTable, array('user_id' => $this->getUserID(), 'test_id' => $this->testID));
    }
    ...
    protected function getReviewFlags($videoID){
        $questionNo = $this->currentVideoNo($videoID);
        if(is_array($_SESSION['hptest'.$this->getTestID()][$questionNo]['clicks'])){$clicks = $_SESSION['hptest'.$this->getTestID()][$questionNo]['clicks'];}
        else{$clicks = unserialize($_SESSION['hptest'.$this->getTestID()][$questionNo]['clicks']);}
        if(is_array($clicks)){
            foreach($clicks as $i => $click){
                $marginleft = ((($click / $this->videoInfo['endClip']) * 100) - 0.1);
                $flags.= '<img src="/images/theory/hpflag.png" alt="Flag" width="20" height="20" id="flag'.($i + 1).'" class="reviewflag" style="left:'.$marginleft.'%" data-click="'.$click.'" />';
            }
        }
        return $flags;
    }
    ...
    protected function clipScore($prim){
        ...
        if($vidInfo['nohazards'] == 1){return '<div class="yourscore">You scored '.$score.' for this hazard</div>';}
        else{return '<div class="yourscore">You scored '.$score.' for the first hazard and '.$_SESSION['hptest'.$this->getTestID()]['second_score'].' for the second hazard</div>';}
    }
    ...
    protected function buildTest($prim = false){
        ...
        self::$template->assign('question_no', $this->currentVideoNo($prim), true);
        self::$template->assign('no_questions', $this->numVideos, true);
        self::$template->assign('video_data', $this->videodata, true);
    ...
    public function endTest($mark){
        ...
        self::$template->assign('windows', $windows, true);
        self::$template->assign('score', $score, true);
        self::$template->assign('passmark', $this->passmark, true);
        self::$template->assign('videos', $videos, true);
        self::$template->assign('testID', $this->getTestID(), true);
    ...
    protected function addResultsToDB(){
        if($this->anyCompleteTests()){self::$db->delete($this->progressTable, array('user_id' => $this->getUserID(), 'test_id' => $this->getTestID()));}
        self::$db->insert(
            $this->progressTable,
            array(
                'user_id' => $this->getUserID(),
                'test_id' => $this->getTestID(),
                'progress' => serialize($_SESSION['hptest'.$this->getTestID()]),
                'test_type' => strtoupper('CAR'),
                'status' => $this->status
            )
        );
    }
