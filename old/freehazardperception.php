...
class FreeHazardPerception extends HazardPerception{
    ...
    /**
     * Returns the JavaScript files needed to complete the current test
     * @return string Returns the JavaScript files needed to complete the current test
     */
    protected function getScript(){
        if($this->report == false){return '<script type="text/javascript" src="/js/theory/hazard-perception-freehazupdate.js"></script>';}
        else{return '<script type="text/javascript" src="/js/theory/hazard-report-freehazupdate.js"></script>';}
    }
    ...
    public function endTest($mark){
        if(!self::$user->getUserID()){self::$template->assign('free_test', 'Yes', true);}
