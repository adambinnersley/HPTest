...
class HazardPerception implements HPInterface
{
    ...
    protected function prevVideo($videoID)
    {
        ...
        if ($prevID >= 1) {
            return $this->getSessionInfo()['videos'][$prevID];
        }
        return 'none';
    }
    ...
    protected function nextVideo($videoID)
    {
        ...
        if ($nextID <= 14) {
            return $this->getSessionInfo()['videos'][$nextID];
        }
        return 'none';
    }
    ...
    protected function markHazard($click, $winNo = 1)
    {
        $score = 5;
        for ($h = 0; $h <= 4; $h++) {
            if ($click >= $this->videoInfo[$this->windows[$winNo][$h]] && $click < $this->videoInfo[$this->windows[$winNo][($h + 1)]]) {
                return $score;
            }
            $score--;
        }
    ...
    protected function clipScore($prim)
    {
        ...
        if (isset($this->getSessionInfo()[$clipNo]['score'])) {
            if ($this->getSessionInfo()[$clipNo]['score'] < 0) {
                $score[] = 0;
            } else {
                $score[] = intval($this->getSessionInfo()[$clipNo]['score']);
            }
        } else {
            $score[] = 0;
        }
        if ($vidInfo['nohazards'] != 1) {
            $score[] = intval(isset($this->getSessionInfo()['second_score']) ? $this->getSessionInfo()['second_score'] : 0);
        }
        return $score;
    }
    ...
    protected function anyCheating($prim)
    {
        ...
        $this->getUserProgress($this->getTestID());
        if (isset($this->getSessionInfo()[$clipNo]['score'])) {
            if ($this->getSessionInfo()[$clipNo]['score'] == -1) {
                return true;
            }
        }
        return false;
    ...
    protected function buildTest($prim = false)
    {
        ...
        if ($this->report === false) {
            return $this->template->fetch('hazardtest.tpl');
        }
    ...
    public function endTest($mark)
    {
        ...
        if ($mark === true) {
            if ($totalScore >= $this->getPassmark()) {
                $this->status = 1;
            } else {
                $this->status = 2;
            }
